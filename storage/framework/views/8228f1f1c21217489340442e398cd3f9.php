...
<?php $__env->startSection('title', 'Kelola Aset Meeting'); ?>
<?php $__env->startSection('page-title', 'Overview > Kelola Aset Meeting'); ?>
<?php $__env->startSection('page-subtitle', 'Inventaris perlengkapan pendukung meeting.'); ?>
...
<div class="pt-2 space-y-4 animate-fade-in">
    ...
    <div class="gaming-card" style="overflow:visible;">
        <form method="GET" action="<?php echo e(route('admin.assets.index')); ?>" id="filter-form">
...
<?php if(auth()->user()->role !== 'gm'): ?><th>Aksi</th><?php endif; ?>
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="color:var(--text-muted);"><?php echo e($assets->firstItem() + $loop->index); ?></td>
                        <td style="color:var(--text-primary);font-weight:500;"><?php echo e($asset->name); ?></td>
                        <td style="color:var(--text-muted);"><?php echo e($asset->description ?? '—'); ?></td>
                        <td>
                            <span class="badge badge-cyan"><?php echo e($asset->quantity); ?></span>
                        </td>
                        <td>
                            <span class="badge <?php echo e($asset->is_active ? 'badge-green' : 'badge-red'); ?>">
                                <?php echo e($asset->is_active ? 'Aktif' : 'Nonaktif'); ?>
...
<?php if(auth()->user()->role !== 'gm'): ?>
                        <td>
                            <div class="flex gap-2">
                                <button type="button" onclick="openEditModal(<?php echo e(json_encode(['id'=>$asset->id,'name'=>$asset->name,'description'=>$asset->description ?? '','quantity'=>$asset->quantity,'is_active'=>$asset->is_active])); ?>)" class="btn btn-secondary btn-sm">Edit</button>
                                <form method="POST" action="<?php echo e(route('admin.assets.destroy', $asset)); ?>" onsubmit="confirmSubmit(event, this)" data-confirm="Hapus aset ini?">
...
<?php endif; ?>
        ...
        <div class="px-5 py-3" style="border-top:1px solid var(--border-color);"><?php echo e($assets->links()); ?></div>
    </div>
</div>
...
<div id="create-modal" class="modal-modern" onclick="if(event.target===this)closeCreateModal()">
    <div class="modal-modern-panel md" onclick="event.stopPropagation()">
        ...
        <form id="create-form" method="POST" action="<?php echo e(route('admin.assets.store')); ?>">
            <?php echo csrf_field(); ?>
            <div class="modal-modern-body space-y-4">
                <div>
                    <label class="gaming-label">Nama Aset <span style="color:#f87171;">*</span></label>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\backend-johenofficesystem\resources\views\admin\assets\index.blade.php ENDPATH**/ ?>