...
<?php $__env->startSection('title', 'Permintaan Meeting'); ?>
<?php $__env->startSection('page-title', 'Permintaan Meeting'); ?>
<?php $__env->startSection('page-subtitle', 'Kelola semua permintaan meeting perusahaan'); ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
                <div class="text-xl font-bold" style="color:var(--text-primary);"><?php echo e($totalMeeting); ?></div>
        ...
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
                <div class="text-xl font-bold" style="color:#fbbf24;"><?php echo e($menungguMeeting); ?></div>
        ...
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
                <div class="text-xl font-bold" style="color:#34d399;"><?php echo e($disetujuiMeeting); ?></div>
        ...
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
                <div class="text-xl font-bold" style="color:#f87171;"><?php echo e($ditolakMeeting); ?></div>
    ...
    <div class="gaming-card" style="overflow:visible;">
        ...
        <div class="px-6 py-3 flex flex-wrap items-center gap-3" style="border-bottom:1px solid var(--border-color);">
            ...
            <div class="flex items-center gap-2 ml-auto">
                <div>
                    <input type="month" id="meeting-month-input" value="<?php echo e($meetingMonth ?? now()->format('Y-m')); ?>" onchange="setMeetingMonth()"
                ...
                <a href="<?php echo e(route('admin.export', ['type' => 'meetings', 'meeting_month' => $meetingMonth ?? now()->format('Y-m')])); ?>"
        ...
        <div class="table-responsive">
            <table class="gaming-table" id="meetings-table" style="width:100%;min-width:750px;">
                ...
                <tbody id="meetings-tbody">
                    ...
                    ?>
                    <tr data-status="<?php echo e($meeting->status); ?>" class="meeting-row">
                        <td style="color:var(--text-muted);font-size:0.8rem;"><?php echo e($loop->iteration); ?></td>
                        <td>
                            <span class="font-medium" style="color:var(--text-primary);font-size:0.8rem;"><?php echo e($meeting->title); ?></span>
                        </td>
                        <td class="meeting-pemohon hidden sm:table-cell">
                            <div class="flex items-center gap-1.5">
                                <div class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0"
                                    ...
                                    <?php echo e(strtoupper(substr($meeting->requester->name, 0, 1))); ?>

                                </div>
                                <span style="color:var(--text-secondary);font-size:0.8rem;"><?php echo e($meeting->requester->name); ?></span>
                            </div>
                        </td>
                        <td class="hidden lg:table-cell"><span style="color:var(--text-secondary);font-size:0.8rem;"><?php echo e($meeting->team->name); ?></span></td>
                        <td>
                            <span style="color:var(--text-secondary);font-size:0.8rem;white-space:nowrap;"><?php echo e($meeting->meeting_date->format('d M Y')); ?></span>
                        </td>
                        <td class="hidden sm:table-cell"><span style="color:var(--text-secondary);font-size:0.8rem;"><?php echo e(substr($meeting->start_time,0,5)); ?>–<?php echo e(substr($meeting->end_time,0,5)); ?></span></td>
                        <td><span class="badge <?php echo e($statusStyle); ?>" style="white-space:nowrap;font-size:0.65rem;"><?php echo e(ucfirst($meeting->status)); ?></span></td>
                        <td class="hidden md:table-cell">
                            <?php if($meeting->queue_position !== null && !in_array($meeting->status, ['pending','rejected','cancelled'])): ?>
                                <span class="badge <?php echo e($queueBadge); ?>" style="white-space:nowrap;font-size:0.65rem;"><?php echo e($rt['label']); ?></span>
                            <?php else: ?>
                        ...
                        <td>
                            <div class="flex items-center gap-1" style="white-space:nowrap;">
                                <button type="button" onclick="showDetail(<?php echo e($meeting->id); ?>)" class="btn btn-secondary btn-sm inline-flex items-center gap-1.5" style="padding:4px 8px;font-size:0.7rem;">
                                ...
                                <div class="relative dropdown-actions">
                                    <button type="button" onclick="toggleActionMenu(event, <?php echo e($meeting->id); ?>)" style="padding:6px 10px;line-height:1;font-size:0.8rem;font-weight:700;border:1px solid var(--border-color);border-radius:8px;background:var(--bg-surface);color:var(--text-primary);cursor:pointer;" title="Aksi">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 5v.01M12 12v.01M12 19v.01"/>
                                        </svg>
                                    </button>
                                    <div id="action-menu-<?php echo e($meeting->id); ?>" style="display:none;position:absolute;top:100%;right:0;min-width:155px;background:var(--bg-surface);border:1px solid var(--border-color);border-radius:10px;padding:4px;z-index:99999;margin-top:4px;box-shadow:0 8px 24px rgba(0,0,0,0.15);">
                                        <button type="button" onclick="showDetail(<?php echo e($meeting->id); ?>)" class="w-full text-left px-2.5 py-1.5 text-xs rounded-md transition flex items-center gap-2" style="color:var(--text-secondary);background:none;border:none;cursor:pointer;" onmouseover="this.style.background='var(--bg-surface-2)'" onmouseout="this.style.background='transparent'">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                        ...
                                        <button type="button" onclick="showEditModal(<?php echo e($meeting->id); ?>)" class="w-full text-left px-2.5 py-1.5 text-xs rounded-md transition flex items-center gap-2" style="color:var(--text-secondary);background:none;border:none;cursor:pointer;" onmouseover="this.style.background='var(--bg-surface-2)'" onmouseout="this.style.background='transparent'">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" onclick="confirmDeleteMeeting(<?php echo e($meeting->id); ?>)" class="w-full text-left px-2.5 py-1.5 text-xs rounded-md transition flex items-center gap-2" style="color:#f87171;background:none;border:none;cursor:pointer;" onmouseover="this.style.background='var(--bg-surface-2)'" onmouseout="this.style.background='transparent'">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <select id="edit-room" name="room_id" required class="gaming-input gaming-select mt-1">
                        ...
                        <?php $__currentLoopData = $rooms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $room): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($room->id); ?>"><?php echo e($room->name); ?> (<?php echo e($room->capacity); ?> orang)</option>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\backend-johenofficesystem\resources\views/admin/meetings/index.blade.php ENDPATH**/ ?>