...
<?php if(auth()->user()->role !== 'gm'): ?>
                        <td>
                            <div class="flex gap-2">
                                <button type="button" onclick="openEditModal(<?php echo e(json_encode(['id'=>$user->id,'name'=>$user->name,'username'=>$user->username,'role'=>$user->role,'team_id'=>$user->team_id,'is_active'=>$user->is_active])); ?>)" class="btn btn-secondary btn-sm">Edit</button>
                                <form method="POST" action="<?php echo e(route('admin.users.destroy', $user)); ?>" onsubmit="confirmSubmit(event, this)" data-confirm="Hapus akun ini?">
...
<script>
var teams = <?php echo json_encode($teams->pluck('id', 'name'), 512) ?>;
