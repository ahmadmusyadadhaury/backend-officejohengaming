...
<?php $__env->startSection('title', 'Meeting Saya'); ?>
<?php $__env->startSection('page-title', 'Meeting Saya'); ?>
<?php $__env->startSection('page-subtitle', 'Kelola seluruh meeting yang kamu ajukan'); ?>
...
<div class="pt-2 space-y-4 animate-fade-in">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        ?>
        <?php $__currentLoopData = $meetingStatCards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $card): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="overflow-x-auto">
            <table class="gaming-table min-w-[700px]" id="meetings-table">
                ...
                <tbody id="meetings-tbody">
                    ...
                    ?>
                    <tr data-meeting-id="<?php echo e($meeting->id); ?>" data-status="<?php echo e($meeting->status); ?>" data-title="<?php echo e(strtolower($meeting->title)); ?>" class="meeting-row">
                        <td style="color:var(--text-muted);font-size:0.8rem;"><?php echo e($loop->iteration); ?></td>
                        <td><span class="font-medium" style="color:var(--text-primary);font-size:0.8rem;"><?php echo e($meeting->title); ?></span></td>
                        <td><span style="color:var(--text-secondary);font-size:0.8rem;"><?php echo e($meeting->meeting_date->format('d M Y')); ?></span></td>
                        <td><span style="color:var(--text-secondary);font-size:0.8rem;"><?php echo e(substr($meeting->start_time,0,5)); ?>–<?php echo e(substr($meeting->end_time,0,5)); ?></span></td>
                        <td class="hidden md:table-cell"><span style="color:var(--text-secondary);font-size:0.8rem;"><?php echo e($meeting->room->name); ?></span></td>
                        <td><span class="badge <?php echo e($statusStyle); ?>" style="font-size:0.65rem;"><?php echo e(ucfirst($meeting->status)); ?></span></td>
                        <td class="hidden md:table-cell">
                            <?php if($meeting->queue_position !== null && !in_array($meeting->status, ['pending','rejected','cancelled'])): ?>
                                <span class="badge <?php echo e($queueBadge); ?>" style="font-size:0.65rem;"><?php echo e($rt['label']); ?></span>
                            <?php else: ?>
                        ...
                        <td>
                            <div class="flex items-center gap-1" style="white-space:nowrap;">
                                <button type="button" onclick="showDetail(<?php echo e($meeting->id); ?>)" class="btn btn-secondary btn-sm" style="display:inline-flex;align-items:center;gap:4px;padding:3px 8px;font-size:0.65rem;" title="Lihat detail">
                                ...
                                <div class="dropdown-wrap" style="position:relative;">
                                    <button type="button" onclick="toggleDropdown(this, <?php echo e($meeting->id); ?>)" class="btn btn-secondary btn-sm" style="padding:3px 6px;font-size:0.7rem;line-height:1;">⋮</button>
                                    <div id="dropdown-<?php echo e($meeting->id); ?>" class="dropdown-menu" style="display:none;position:absolute;top:100%;right:0;z-index:99999;min-width:130px;background:var(--bg-surface);border:1px solid var(--border-color);border-radius:10px;padding:4px;box-shadow:0 8px 24px rgba(0,0,0,0.15);margin-top:4px;">
                                        <button type="button" onclick="showDetail(<?php echo e($meeting->id); ?>)" style="display:flex;align-items:center;gap:6px;width:100%;text-align:left;padding:6px 10px;border:none;background:none;font-size:12px;color:var(--text-primary);border-radius:6px;cursor:pointer;" onmouseover="this.style.background='var(--bg-surface-2)'" onmouseout="this.style.background='none'">
                                            <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                            Detail
                                        </button>
                                        <?php if(in_array($meeting->status, ['approved','confirmed','in_progress'])): ?>
                                        <button type="button" onclick="submitAction(<?php echo e($meeting->id); ?>,'cancel')" style="display:block;width:100%;text-align:left;padding:6px 10px;border:none;background:none;font-size:12px;color:#f59e0b;border-radius:6px;cursor:pointer;" onmouseover="this.style.background='var(--bg-surface-2)'" onmouseout="this.style.background='none'">Batalkan</button>
                                        <?php endif; ?>
                                        <button type="button" onclick="submitAction(<?php echo e($meeting->id); ?>,'delete')" style="display:block;width:100%;text-align:left;padding:6px 10px;border:none;background:none;font-size:12px;color:#ef4444;border-radius:6px;cursor:pointer;" onmouseover="this.style.background='var(--bg-surface-2)'" onmouseout="this.style.background='none'">Hapus</button>
                                    </div>
                                </div>
                            </div>
                        </td>
...
<div id="request-modal" style="display:none;position:fixed;inset:0;z-index:99999;align-items:center;justify-content:center;padding:16px;overflow-y:auto;background:var(--bg-overlay);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);">
    <div class="w-full max-w-[820px] max-h-[85vh] rounded-3xl shadow-2xl flex flex-col overflow-y-auto min-h-0" style="background:var(--bg-surface);" onclick="event.stopPropagation()">
        ...
        <form method="POST" action="<?php echo e(route('koordinator.meetings.store')); ?>" enctype="multipart/form-data" class="px-6 py-5 space-y-4 overflow-y-auto flex-1 min-h-0">
            <?php echo csrf_field(); ?>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="start_time" id="modal-start-time" value="<?php echo e(old('start_time')); ?>" required class="gaming-input" placeholder="--:--" autocomplete="off">
                                </div>
                            </div>
                            <div>
                                <label class="gaming-label">Jam Selesai <span style="color:#f87171;">*</span></label>
                                <input type="text" name="end_time" id="modal-end-time" value="<?php echo e(old('end_time')); ?>" required class="gaming-input" placeholder="--:--" autocomplete="off">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <div>
                                ...
                                <textarea name="why" rows="2" required placeholder="Kenapa meeting ini diadakan?" class="gaming-input" style="resize:vertical;"><?php echo e(old('why')); ?></textarea>
                            </div>
                            <div>
                                <label class="gaming-label">WHAT <span style="color:#f87171;">*</span></label>
                                <textarea name="what" rows="2" required placeholder="Apa yang akan dibahas?" class="gaming-input" style="resize:vertical;"><?php echo e(old('what')); ?></textarea>
                            </div>
                            <div>
                                <label class="gaming-label">HOW <span style="color:#f87171;">*</span></label>
                                <textarea name="how_expected" rows="2" required placeholder="Hasil yang diharapkan?" class="gaming-input" style="resize:vertical;"><?php echo e(old('how_expected')); ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="border-top:1px solid var(--border-color);padding-top:0.75rem;">
                        ...
                        <div class="space-y-2">
                            ...
                            <div>
                                ...
                                <div class="space-y-1">
                                    <?php $__currentLoopData = $assets->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $asset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="flex items-center gap-2">
                                        <span class="text-xs flex-1" style="color:var(--text-secondary);"><?php echo e($asset->name); ?></span>
            ...
            <div>
                ...
                <select id="mom-pic" class="gaming-input gaming-select">
                    ...
                    <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($u->name); ?>"><?php echo e($u->name); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div>
...
const usersData = <?php echo json_encode($users->map(fn($u) => ['id' => $u->id, 'name' => $u->name]), 512) ?>;
...
(function() {
    const momId = '<?php echo e(session('mom_meeting_id')); ?>';
    if (momId) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\backend-johenofficesystem\resources\views/leader/meetings/index.blade.php ENDPATH**/ ?>