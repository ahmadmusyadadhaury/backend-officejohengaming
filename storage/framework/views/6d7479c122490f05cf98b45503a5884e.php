...
<?php if(auth()->user()->role !== 'gm'): ?>
                        <td>
                            <div class="flex gap-2">
                                <button type="button" onclick="openEditModal(<?php echo e(json_encode(['id'=>$team->id,'name'=>$team->name,'description'=>$team->description ?? '','is_active'=>$team->is_active])); ?>)" class="btn btn-secondary btn-sm">Edit</button>
                                <form method="POST" action="<?php echo e(route('admin.teams.destroy', $team)); ?>" onsubmit="confirmSubmit(event, this)" data-confirm="Hapus tim ini?">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\backend-johenofficesystem\resources\views\admin\teams\index.blade.php ENDPATH**/ ?>