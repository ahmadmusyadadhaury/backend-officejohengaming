...
<body style="font-family:sans-serif;background:#f4f4f4;padding:40px;">
    <div style="max-width:520px;margin:auto;background:#fff;border-radius:12px;padding:32px;">
        ...
        <p style="margin:0 0 6px;font-size:14px;color:#333;">
            Kendaraan: <strong style="color:#6c5cff;">{{ $vehicle->nama_kendaraan }}</strong>
        </p>
        <p style="margin:0 0 6px;font-size:14px;color:#333;">
            Plat Nomor: <strong style="color:#6c5cff;">{{ $vehicle->plat_nomor }}</strong>
        </p>
        <p style="margin:0 0 6px;font-size:14px;color:#333;">
            Jenis Pajak: <strong style="color:#6c5cff;">{{ $request->jenis === 'tahunan' ? 'Pajak Tahunan' : 'Pajak 5 Tahunan' }}</strong>
        </p>
        <p style="margin:0 0 6px;font-size:14px;color:#333;">
            Nominal: <strong style="color:#6c5cff;">Rp {{ number_format($request->nominal, 0, ',', '.') }}</strong>
        </p>
        <p style="margin:0 0 20px;font-size:14px;color:#333;">
            Diajukan Oleh: <strong style="color:#6c5cff;">{{ $requester->name }} ({{ $requester->role }})</strong>
        </p>
        <p style="margin:0 0 20px;font-size:14px;color:#333;">Terima kasih.</p>
        <a href="{{ $url }}" style="display:inline-block;padding:10px 20px;background:#6c5cff;color:#fff;text-decoration:none;border-radius:8px;font-size:14px;">Lihat & Approve Pengajuan</a>
