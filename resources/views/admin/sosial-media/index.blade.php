...
<div class="pt-2 space-y-5 animate-fade-in">
        ...
        <div style="flex:1;min-width:260px;">
            <div class="flex items-start gap-3 px-5 py-3.5 rounded-2xl" style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);">
                ...
                <div class="flex-1 min-w-0">
                    <div class="text-sm font-bold" style="color:#ef4444;">{{ $alertItems->count() }} Akun Nonaktif</div>
                    <div class="text-xs mt-1" style="color:var(--text-secondary);">{{ $alertItems->count() }} akun sosial media berstatus tidak aktif.</div>
                </div>
    ...
    <div class="gaming-card" style="overflow:hidden;">
        <div class="px-6 py-4 flex items-center justify-between" style="border-bottom:1px solid var(--border-color);">
            <div>
                <div style="font-weight:600;font-size:0.8rem;color:var(--text-primary);">Data Sosial Media</div>
                <div style="font-size:0.7rem;color:var(--text-muted);margin-top:2px;font-weight:400;">Akun sosial media operasional perusahaan.</div>
            </div>
        ...
        <div class="px-6 py-2.5 flex flex-wrap items-center gap-3" style="border-bottom:1px solid var(--border-color);">
            <div class="relative flex-1 min-w-[200px] max-w-[260px]">
                ...
                <form method="GET" action="{{ route('admin.sosial-media.index') }}">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari username, nama, atau platform" oninput="this.form.submit()"
                        class="w-full pl-9 pr-3 py-1.5 rounded-lg text-xs"
                        style="background:var(--bg-surface);border:1px solid var(--border-color);color:var(--text-primary);outline:none;">
                </form>
            </div>
            <div class="flex items-center gap-2" style="margin-left:auto;">
                <a href="{{ route('admin.export', ['type' => 'sosial-media', 'filter' => 'all']) }}" class="btn btn-secondary btn-sm inline-flex items-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Export</a>
                ...
                <button type="button" onclick="toggleFilterMenu(event)" class="filter-btn"
                    ...
                    <span id="filter-label">{{ $status && $status !== 'all' ? $status : 'Semua Platform' }}</span>
...
<div id="alert-overlay" style="display:none;position:fixed;inset:0;z-index:9999;background:var(--bg-overlay);align-items:center;justify-content:center;padding:16px;" onclick="if(event.target===this)closeAlertPopup()">
    <div style="background:var(--bg-surface);border-radius:16px;padding:24px;width:90%;max-width:460px;max-height:65vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            <div id="alert-popup-title" style="font-weight:700;font-size:16px;color:var(--text-primary);">Akun Sosial Media Nonaktif</div>
...
function showAlertPopup() {
    ...
    title.textContent = 'Akun Sosial Media Nonaktif';
    const items = alertData.filter(function(i) { return i.status === 'nonaktif'; });

    if (items.length === 0) {
        body.innerHTML = '<p style="text-align:center;padding:20px;color:var(--text-muted);">Tidak ada akun nonaktif.</p>';
    } else {
        body.innerHTML = items.map(function(i) {
            return '<div class="flex items-center gap-3 px-4 py-3.5 rounded-xl" style="border:1px solid var(--border-color);margin-bottom:8px;cursor:pointer;transition:all 0.15s;background:var(--bg-surface-2);" onclick="openEditModal(' + i.id + ')" onmouseover="this.style.borderColor=\'' + color + '\'" onmouseout="this.style.borderColor=\'var(--border-color)\'">' +
                '<div class="flex-1 min-w-0">' +
                    '<p style="font-weight:600;font-size:14px;color:var(--text-primary);margin:0;">' + i.username + '</p>' +
                    '<p style="font-size:12px;color:var(--text-muted);margin:2px 0 0;">' + i.nama + ' — ' + i.platform + '</p>' +
                    '<p style="font-size:11px;color:var(--text-secondary);margin:4px 0 0;">PIC: ' + (i.pic || '-') + '</p>' +
                '</div>' +
                '<span onclick="event.stopPropagation()"><button onclick="openEditModal(' + i.id + ')" style="flex-shrink:0;padding:8px 16px;border-radius:10px;font-size:12px;font-weight:700;border:none;cursor:pointer;transition:all 0.2s;background:' + color + ';color:#fff;box-shadow:0 4px 12px rgba(239,68,68,0.3);" onmouseover="this.style.opacity=\'0.85\'" onmouseout="this.style.opacity=\'1\'">Aktifkan</button></span>' +
            '</div>';
        }).join('');
    }
