    ...
    <div class="gaming-card" style="overflow:visible;">
        ...
        <div class="px-5 py-2.5 flex flex-wrap items-center gap-3" style="border-bottom:1px solid var(--border-color);">
            ...
            <div class="flex items-center gap-2" style="margin-left:auto;">
                @if($jenis === 'ipl_ruko')
                <form method="GET" action="{{ route('admin.pembayaran.index') }}" class="flex items-center gap-1.5" style="margin:0;">
                    <input type="hidden" name="jenis" value="ipl_ruko">
                    <select name="tahun" onchange="this.form.submit()" class="gaming-input" style="padding:6px 10px;font-size:12px;width:auto;cursor:pointer;">
                        @foreach($availableYears as $yr)
                            <option value="{{ $yr }}" {{ (string) $yr === (string) $tahun ? 'selected' : '' }}>{{ $yr }}</option>
                        @endforeach
