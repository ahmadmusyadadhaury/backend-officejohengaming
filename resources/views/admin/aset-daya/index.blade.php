@extends('layouts.app')
@section('title', 'Aset Daya')
@section('page-title', 'Data Aset > Aset Daya')
@section('page-subtitle', 'Daftar aset daya dan utilitas perusahaan')
...
<div class="pt-2 space-y-4 animate-fade-in">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                <div class="text-[11px] font-medium mt-0.5" style="color:var(--text-primary);">Total Aset Daya</div>
                <div class="text-[11px] mt-0.5 leading-tight" style="color:var(--text-muted);">Seluruh aset daya perusahaan</div>
            </div>
        </div>
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <div class="text-[11px] mt-0.5 leading-tight" style="color:var(--text-muted);">Aset daya yang masih aktif</div>
            </div>
        </div>
        <div class="gaming-card p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <div class="text-[11px] mt-0.5 leading-tight" style="color:var(--text-muted);">Aset daya yang tidak aktif</div>
            </div>
        </div>
    </div>
    ...
    <div class="gaming-card" style="overflow:visible;">
        <div class="px-6 py-4 flex items-center justify-between" style="border-bottom:1px solid var(--border-color);">
            <div>
                <div style="font-weight:600;font-size:0.8rem;color:var(--text-primary);">Data Aset Daya</div>
                <div style="font-size:0.7rem;color:var(--text-muted);margin-top:2px;font-weight:400;">Aset daya dan utilitas perusahaan.</div>
            </div>
        ...
        <div class="table-responsive">
            <table class="gaming-table min-w-[700px]" id="aset-table">
                ...
                <tbody id="aset-tbody">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <div class="dropdown-wrap" style="position:relative;">
                                    ...
                                    <div id="dropdown-{{ $a->id }}" class="dropdown-menu" style="display:none;position:absolute;top:100%;right:0;z-index:99999;min-width:130px;background:var(--bg-surface);border:1px solid var(--border-color);border-radius:10px;padding:4px;box-shadow:0 8px 24px rgba(0,0,0,0.15);margin-top:4px;">
                                        ...
                                        <form method="POST" action="{{ route('admin.aset-daya.destroy', $a) }}" onsubmit="confirmSubmit(event, this)" data-confirm="Hapus aset daya ini?" style="margin:0;">
                                            @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--text-muted);">Belum ada data aset daya.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<div id="detail-modal" style="display:none;position:fixed;inset:0;z-index:50;align-items:center;justify-content:center;padding:16px;background:var(--bg-overlay);">
    <div class="w-full max-w-[460px] rounded-3xl shadow-2xl flex flex-col" style="max-height:65vh;background:var(--bg-surface);" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between px-6 py-4 flex-shrink-0" style="border-bottom:1px solid var(--border-color);">
            <h3 class="text-base font-bold" style="color:var(--text-primary);" id="detail-title">Detail Aset Daya</h3>
...
<div id="aset-modal" style="display:none;position:fixed;inset:0;z-index:50;align-items:center;justify-content:center;padding:16px;background:var(--bg-overlay);">
    <div class="w-full max-w-[480px] rounded-3xl shadow-2xl flex flex-col" style="max-height:65vh;background:var(--bg-surface);" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between px-6 py-4 flex-shrink-0" style="border-bottom:1px solid var(--border-color);">
            <h3 class="text-base font-bold" style="color:var(--text-primary);" id="modal-title">Tambah Aset Daya</h3>
        ...
        <div class="px-6 py-5 overflow-y-auto flex-1">
            <form id="aset-form" method="POST">
                ...
                <div class="form-grid-2">
                    ...
                    <div class="field-group">
                        <label class="gaming-label">Daya</label>
                        <input type="text" name="daya" id="f-daya" placeholder="Contoh: 2200 VA" class="gaming-input">
                    </div>
                    <div class="field-group">
                        <label class="gaming-label">Unit / Lokasi</label>
                        <input type="text" name="unit" id="f-unit" placeholder="Lokasi aset" class="gaming-input">
                    </div>
                    <div class="field-group">
...
function showDetail(id) {
    ...
    if (!a) return;
    document.getElementById('detail-title').textContent = 'Detail Aset Daya';
    document.getElementById('detail-body').innerHTML = `
        <div class="detail-grid">
            <div class="detail-item"><span class="detail-label">Nama Aset</span><span class="detail-value">${a.nama_aset}</span></div>
            <div class="detail-item"><span class="detail-label">Daya</span><span class="detail-value">${a.daya || '-'}</span></div>
            <div class="detail-item"><span class="detail-label">Unit</span><span class="detail-value">${a.unit || '-'}</span></div>
...
function openCreateModal() {
    document.getElementById('modal-title').textContent = 'Tambah Aset Daya';
    document.getElementById('form-method').value = 'POST';
    document.getElementById('form-id').value = '';
    document.getElementById('form-submit-btn').textContent = 'Tambah';
    document.getElementById('aset-form').action = '{{ route("admin.aset-daya.index") }}';
    document.getElementById('f-nama_aset').value = '';
    document.getElementById('f-daya').value = '';
...
function openEditModal(id) {
    const a = assets.find(x => x.id === id);
    if (!a) return;
    document.getElementById('modal-title').textContent = 'Edit Aset Daya';
    document.getElementById('form-method').value = 'PUT';
    document.getElementById('form-id').value = a.id;
    document.getElementById('form-submit-btn').textContent = 'Simpan';
    document.getElementById('aset-form').action = '{{ url("admin/aset-daya") }}/' + a.id;
    document.getElementById('f-nama_aset').value = a.nama_aset;
    document.getElementById('f-daya').value = a.daya || '';
    document.getElementById('f-unit').value = a.unit || '';
