                ...
                @forelse($pendingMeetings as $meeting)
                    <div class="px-6 py-3 flex items-center justify-between">
                        ...
                        <a href="{{ route('admin.meetings.show', $meeting) }}"
                            class="px-3 py-1 bg-accent/10 text-accent rounded-lg text-xs font-medium hover:bg-accent hover:text-white transition">
                            Review
                        </a>
                    </div>
                @empty
                    <p class="px-6 py-4 text-sm text-gray-400">Tidak ada permintaan pending.</p>
                @endforelse
            </div>
        </div>
                    ...
                    <div class="px-6 py-3">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="w-2 h-2 rounded-full
                                {{ $meeting->status === 'in_progress' ? 'bg-accent' : 'bg-secondary' }}"></span>
                            <p class="text-sm font-medium text-gray-800">{{ $meeting->title }}</p>
                        </div>
