        ...
        <form method="POST" action="{{ route('admin.admins.update', $admin) }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="role" required class="gaming-input gaming-select">
                    ...
                    <option value="ceo"           {{ $admin->role === 'ceo'           ? 'selected' : '' }}>Chief Executive Officer (CEO)</option>
                    <option value="hr"            {{ $admin->role === 'hr'            ? 'selected' : '' }}>HR (Human Resources)</option>
                    <option value="assistant_manager" {{ $admin->role === 'assistant_manager' ? 'selected' : '' }}>Asisten Manager</option>
                    <option value="admin_ga"      {{ $admin->role === 'admin_ga'      ? 'selected' : '' }}>Admin General Affairs</option>
                </select>
            </div>
