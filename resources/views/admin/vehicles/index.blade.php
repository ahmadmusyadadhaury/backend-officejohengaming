                ...
                <tbody id="vehicles-tbody">
                    ...
                    <tr data-status="{{ $v->status_pajak }}">
                        ...
                        <td class="hidden lg:table-cell" style="vertical-align:middle;font-size:12px;">
                            @if($v->foto)
                            <a href="{{ route('files.show', $v->foto) }}" target="_blank" rel="noopener">
                                <img src="{{ route('files.show', $v->foto) }}" alt="Foto" style="width:60px;height:40px;border-radius:6px;object-fit:cover;border:1px solid var(--border-color);display:block;">
                            </a>
                            @else
        ...
        <div class="space-y-1">
            ${rows.map((r, i) => `
                <div class="flex items-center justify-between py-2.5" ${i < rows.length - 1 ? 'style="border-bottom:1px solid var(--border-color);"' : ''}>
                    ...
                    <p class="text-sm font-semibold text-right" style="color:var(--text-primary);max-width:55%;">${r.label === 'Plat Nomor' ? `<span style="font-family:monospace;background:var(--bg-surface-2);padding:2px 8px;border-radius:6px;">${r.value}</span>` : r.value}</p>
                </div>
            `).join('')}
            <div class="flex items-center justify-between py-2.5">
                <p class="text-sm" style="color:var(--text-muted);">Status</p>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" style="background:${st.bg};color:${st.text};border:1px solid ${st.border};">${st.label}${v.hari_pajak ? ' (' + v.hari_pajak + ')' : ''}</span>
            </div>
        </div>
...
function closeDetail() {
    ...
    document.getElementById('detail-modal').style.zIndex = '';
}
